...
export function normalizeUserProfile(id: string, data: Record<string, unknown>): UserProfile {
  ...
  return {
    ...
    namaDepan: strField(data, 'namaDepan', 50),
    namaBelakang: strField(data, 'namaBelakang', 50),
    tempatLahir: strField(data, 'tempatLahir', 80),
    alamat: strField(data, 'alamat', 500),
    agama: strField(data, 'agama', 50),
    noHp: strField(data, 'noHp', 20),
    tahunMasuk: strField(data, 'tahunMasuk', 4),
    membershipStatus: data.membershipStatus === 'purna' ? 'purna' : data.membershipStatus === 'aktif' ? 'aktif' : undefined,
